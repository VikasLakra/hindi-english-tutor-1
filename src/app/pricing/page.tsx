      ...
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-14 grid gap-5 md:grid-cols-[1.15fr_.85fr]">
          <div className="border border-border bg-foreground p-7 text-background sm:p-9">
            ...
            <div className="mt-10 flex items-end gap-2"><strong className="text-6xl leading-none tracking-[-0.07em]">$9.99</strong><span className="pb-1 text-background/55">/ month</span></div>
            <div className="mt-9 grid gap-3 border-t border-background/20 pt-6 text-sm text-background/80 sm:grid-cols-2"><p>✓ Advanced daily lessons</p><p>✓ Unlimited tutor practice</p><p>✓ Speaking practice cues</p><p>✓ Personalized progress</p></div>
            <div className="mt-9">{user ? <SubscribeButton /> : <Link href="/auth/start" className="inline-flex h-9 items-center gap-2 bg-primary px-4 text-sm font-bold text-primary-foreground">Sign in to subscribe <span>↗</span></Link>}<p className="mt-3 text-xs text-background/45">Cancel anytime. Secure checkout handled by Stripe.</p></div>
          </div>
